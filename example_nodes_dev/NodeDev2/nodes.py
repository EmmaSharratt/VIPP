...
class Welcome(Node):
    ...
    def update_event(self, inp=-1):         
        ...
        new = cv2.putText(blank, 'How to add Text', (100, 400), cv2.FONT_HERSHEY_PLAIN, 1.0, (255, 255, 255), 2)
        # Show or save the image with the added text as per your requirement
        return new
        # blank.show()
...
class New(Node):
    ...
    def update_event(self, inp=-1):         
        ...
        # Put the text on the image
        cv2.putText(blank, 'How to add Text', (100, 400), cv2.FONT_HERSHEY_PLAIN, 1.0, (255, 255, 255), 2)
        # Show or save the image with the added text as per your requirement
        return self.input(0).blank
        # print(self.input(0))
    
class Hello(OpenCVNodeBase):
    title = 'Hello'
    init_inputs = [
        NodeInputBP(),
    ]
...
class WelcomeText(OpenCVNodeBase):
    title = 'WELCOME'
    init_inputs = [
        NodeInputBP('img'),
    ]
    ...
    def get_img(self):
        # input_node = self.input(0)
        # image = input_node.get_value()

        # Create a blank image to draw the text on
        # image_with_text = Image.new("RGB", (200, 200), (0, 0, 0))

        # Define the text and position
        # text = 'How to add Text'
        # position = (100, 100)  # Adjust the position as per your requirement

        # Draw the text on the image
        # self.draw_text(image_with_text, text, position)

        # Show or save the image with the added text as per your requirement
        # image_with_text.show()

        # Return the original image as the output
        return cv2.putText(
            img=self.input(0).img.copy(), 
...
# class Hello(Node):
#     blk = np.zeros((200,200,3), dtype = 'uint8')
#     title = 'Hello'
#     init_inputs = [
#         NodeInputBP(),
#     ]
...
#         blk = np.zeros((200,200,3), dtype = 'uint8')
#         # Put the text on the image        
#         return cv2.putText(blk, 'How to add Text', (100, 400), cv2.FONT_HERSHEY_PLAIN, 1.0, (255, 255, 255), 2)
