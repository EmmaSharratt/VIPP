...
# Base classes: 
# Different image processing techniques are applied slightly differentlt in some cases (hence a different base class)
# And different groupd of image processing techniques are assigned different colours
...
class NodeBase0(Node):
    ...
    def handle_stack(self):
        self.image_stack = self.input(0)[0] # image [Z, H, W, C]
        self.stack_dict = self.input(0)[1] # dictioary
        self.z_sclice = self.input(0)[2] # keep track of which z-slice is displayed
...
class NodePipeline(Node): 
    ...
    def get_img(self, zslice):
        ...
        # Ensures [Z H W 1] one at the end stays 
        processed_slice = processed_data.reshape(zslice.shape)
        
        return processed_slice
    
    # Slice by sclice operations
    # Signle time step
...
class NodeBase2(Node):
    version = 'v0.1'
    color = '#92D050' #green - Binarization (have to reshape to add a channel for standarzation)
    ...
    def proc_stack_parallel(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
            ...
            for z, future in futures:
                processed_frame = future.result()
                #Binaraization have to reshape to add a channel for standarzation
                proc_data[z] = processed_frame
...
class CVImage: # This is from Ryven original
    """
...
class SaveImg(NodeBase0):
    ...
    def path_chosen(self, new_path):
        ...
        if self.input(0)[0].shape[3] == 1:
            ...
            metadata = {
                'Description': 'Stack (grayscale) preprocessed with Visual Processing Pipeline. Developed using Ryven at Stellenbosch University by Emma Sharratt and Dr Rensu Theart',
        ...
        if self.input(0)[0].shape[3] == 3:
            ...
            custom_metadata = {
                "Description": "Stack (RGB) preprocessed with Visual Processing Pipeline. Developed using Ryven by Emma Sharratt and Dr Rensu Theart",
                "Author": "Emma Sharratt and Dr Rensu Theart",
                "Date": "Pipeline created in 2023",
                'axes': 'ZCYX'
...
class BatchProcess(NodeBase0):
    ...
    def save_stack(self, stack):
        ...
        if stack.shape[0]>1 :
            ...
            grayscale_stack = stack[:, :, :, :, 0]
            custom_metadata = {
                "Description": "Stack (grayscalse) preprocessed with Visual Processing Pipeline. Developed using Ryven by Emma Sharratt and Dr Rensu Theart",
                "Author": "Emma Sharratt and Dr Rensu Theart",
                "Date": "Pipeline created in 2023",
                'axes': 'TZYX'
                # "256": RGB_stack.shape[2], #W
...
# APPLY TO WHOLE STACK AT ONCE (volume operation not slice by sclice)
# Therefore we don't use Nodepipeline baseclass proc_technique()
# Similar to Volume_filter node
...
# APPLY TO WHOLE STACK AT ONCE (volume operation not slice by sclice)
# Therefore we don't use Nodepipeline baseclass proc_technique()
# Similar to Gaussian_Blur3D node - apply techniques to whole volume
class Volume_filter(NodeBase): 
    title = 'Volume Filter'
    version = 'v0.1'
    init_inputs = [
...
# Checkpoint_Node also used in VIPP
